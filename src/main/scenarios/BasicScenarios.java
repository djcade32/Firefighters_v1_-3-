...
public class BasicScenarios {
  @Test
  public void singleFire() throws FireproofBuildingException, OutOfCityBoundsException, NoFireFoundException {
  ...
  public void singleFireDistanceTraveledDiagonal()
      throws FireproofBuildingException, OutOfCityBoundsException, NoFireFoundException {
  ...
  @Test
  public void simpleDoubleFire() throws FireproofBuildingException, OutOfCityBoundsException, NoFireFoundException {
    City basicCity = new CityImpl(2, 2, new CityNode(0, 0));
    FireDispatch fireDispatch = basicCity.getFireDispatch();
  ...
  @Test
  public void optimizingFireFighterUsed()
      throws FireproofBuildingException, OutOfCityBoundsException, NoFireFoundException {
    City basicCity = new CityImpl(3, 3, new CityNode(0, 0));
    FireDispatch fireDispatch = basicCity.getFireDispatch();

    CityNode[] fireNodes = { new CityNode(0, 1), new CityNode(1, 0), new CityNode(0, 2) };
    Pyromaniac.setFires(basicCity, fireNodes);
    ...
    Assert.assertEquals(3, totalDistanceTraveled);
    Assert.assertTrue(firefighterPresentAtFireTwo);
    Assert.assertTrue(firefighterPresentAtFireThree);
    Assert.assertFalse(basicCity.getBuilding(fireNodes[0]).isBurning());
    Assert.assertFalse(basicCity.getBuilding(fireNodes[1]).isBurning());
    Assert.assertFalse(basicCity.getBuilding(fireNodes[2]).isBurning());
    Assert.assertEquals(fireNodes[2], firefighters.get(0).getLocation());
    Assert.assertEquals(basicCity.getFireStation().getLocation(), firefighters.get(2).getLocation());
  }
}
